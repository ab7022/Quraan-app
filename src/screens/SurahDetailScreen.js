...
export default function SurahDetailScreen({ surahId, name, onBack }) {
  ...
  async function fetchVerses() {
    setLoading(true);
    try {
      const res = await fetch(`https://api.alquran.cloud/v1/surah/${surahId}/quran-uthmani-quran-academy`);
  ...
  return (
    <View style={tw`flex-1 bg-white dark:bg-black`}> 
      <View style={tw`flex-row items-center px-4 pt-7 pb-2 bg-white dark:bg-black`}> 
        ...
        <Text style={tw`text-lg font-bold text-black dark:text-white`}>{surahInfo?.englishName || name}</Text>
      </View>
      {loading ? (
        ...
        <ScrollView style={tw`flex-1`} contentContainerStyle={tw`px-5 pb-10`}>
          <Text style={tw`text-center text-2xl font-bold text-green-900 dark:text-green-200 mb-4 mt-3`}>{surahInfo?.name}</Text>
          <Text style={tw`text-center text-base text-gray-700 dark:text-gray-300 mb-6`}>{surahInfo?.englishNameTranslation} | {surahInfo?.numberOfAyahs} Ayah</Text>
          {verses.map((v, index) => (
            <View key={`verse-${v.number || `${v.numberInSurah}-${index}`}`} style={tw`mb-6`}> 
              ...
              <Text style={tw`text-xs text-gray-500 dark:text-gray-400 text-right mt-1`}>Ayah {v.numberInSurah}</Text>
            </View>
          ))}
        </ScrollView>
      )}
