...
const Last30DaysStreak = () => {
  const { readingHistory, totalDaysRead, longestStreak } = useSelector(
    s => s.streak
  );
  ...
  const getStreakColor = (hasRead, isToday) => {
    ...
    if (isToday && !hasRead)
      return 'bg-gray-300 dark:bg-gray-600 border-green-500 border-2';
    ...
    <View style={tw`bg-white dark:bg-gray-800 rounded-xl p-4 mb-4 shadow-sm`}>
      <View style={tw`flex-row items-center justify-between mb-4`}>
        ...
        <View style={tw`flex-row items-center`}>
          ...
          <Text
            style={tw`text-sm font-medium text-green-600 dark:text-green-400 ml-1`}
          >
            {readingPercentage}%
          </Text>
        </View>
      ...
      {/* Stats Row */}
      <View
        style={tw`flex-row justify-between mb-4 bg-gray-50 dark:bg-gray-900 rounded-lg p-3`}
      >
        <View style={tw`items-center`}>
          <Text
            style={tw`text-2xl font-bold text-green-600 dark:text-green-400`}
          >
            {last30DaysReadCount}
          </Text>
        ...
        <View style={tw`items-center`}>
          <Text
            style={tw`text-2xl font-bold text-purple-600 dark:text-purple-400`}
          >
            {longestStreak}
          </Text>
          <Text style={tw`text-xs text-gray-500 dark:text-gray-400`}>
        ...
        <View style={tw`flex-row justify-between mb-2`}>
          {last30Days.slice(0, 8).map((day, index) => (
            <View key={day.date} style={tw`items-center flex-1`}>
              ...
              <Text
                style={tw`text-xs ${getTextColor(day.hasRead, day.isToday)} font-medium`}
              >
                {day.dayNumber}
              </Text>
            </View>
      ...
      {/* Legend */}
      <View
        style={tw`flex-row justify-center items-center mt-4 pt-3 border-t border-gray-200 dark:border-gray-700`}
      >
        ...
        <View style={tw`flex-row items-center mr-4`}>
          <View
            style={tw`w-3 h-3 bg-gray-200 dark:bg-gray-700 border-2 border-red-400 rounded-full mr-1`}
          />
          <Text style={tw`text-xs text-gray-500 dark:text-gray-400`}>
            Missed
          </Text>
        ...
        <View style={tw`flex-row items-center`}>
          <View
            style={tw`w-3 h-3 bg-gray-300 dark:bg-gray-600 border-2 border-green-500 rounded-full mr-1`}
          />
          <Text style={tw`text-xs text-gray-500 dark:text-gray-400`}>
            Today
          </Text>
