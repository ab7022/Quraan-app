...
const StreakCalendar = () => {
  ...
  if (!readingHistory || typeof readingHistory !== 'object') {
    return (
      <View style={tw`bg-white dark:bg-gray-800 rounded-xl p-4 mb-4 items-center justify-center h-48`}>
        <Text style={tw`text-gray-500 dark:text-gray-400`}>Loading reading history...</Text>
      </View>
    );
  }
    ...
    <View style={tw`bg-white dark:bg-gray-800 rounded-xl p-4 mb-4`}>
      ...
      <View style={tw`flex-row justify-between mb-2 px-1`}>
        {months.map((month, index) => (
          <Text key={month} style={tw`text-xs text-gray-500 dark:text-gray-400`}>
            {index % 3 === 0 ? month : ''}
          </Text>
        ))}
      </View>
            ...
            <View key={weekIndex} style={tw`mr-1`}>
              {week.map((day, dayIndex) => (
                <View
                  ...
                  style={[
                    tw`w-3 h-3 rounded-sm mb-1 ${getSquareColor(day)}`,
                  ]}
                />
              ))}
            </View>
      ...
      <View style={tw`flex-row items-center justify-between mt-3`}>
        ...
        <View style={tw`flex-row items-center`}>
          <View style={tw`w-3 h-3 bg-gray-200 dark:bg-gray-700 rounded-sm mr-1`} />
      ...
      {/* Stats */}
      <View style={tw`flex-row justify-between mt-4 pt-3 border-t border-gray-200 dark:border-gray-600`}>
        ...
        <View style={tw`items-center`}>
          <Text style={tw`text-lg font-bold text-purple-600 dark:text-purple-400`}>
            {Math.round((Object.values(readingHistory).filter(Boolean).length / 365) * 100)}%
          </Text>
          <Text style={tw`text-xs text-gray-500 dark:text-gray-400`}>
            This Year
          </Text>
